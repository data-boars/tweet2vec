...
from .settings_word import MAX_CLASSES, N_BATCH, N_WORD
from .w2v import load_params, tweet2vec
...
def main(args):
    ...
    n_char = min(len(list(chardict.keys())) + 1, N_WORD)
    ...
    for i in range(numbatches):
        ...
        for idx, item in enumerate(xr):
            out_pred.append(
                " ".join([inverse_labeldict[r] for r in ranks[idx, :5]])
            )
