...
from . import batch_word as batch
...
from .w2v import init_params, load_params_shared, tweet2vec
...
def main(train_path, val_path, save_path, num_epochs=NUM_EPOCHS):
    ...
    if not RELOAD_MODEL:
        ...
        n_token = min(len(list(tokendict.keys())) + 1, N_WORD)
        ...
        batch.save_dictionary(
            labeldict, labelcount, "%s/label_dict.pkl" % save_path
        )
        ...
        params["W_cl"] = theano.shared(
            np.random.normal(
                loc=0., scale=SCALE, size=(WDIM, n_classes)
            ).astype("float32"),
            name="W_cl",
        )
    ...
    train_iter = batch.BatchTweets(
        Xt, yt, labeldict, batch_size=N_BATCH, max_classes=MAX_CLASSES
    )
    val_iter = batch.BatchTweets(
        Xv,
        yv,
        labeldict,
        batch_size=N_BATCH,
    ...
    try:
        for epoch in range(num_epochs):
            ...
            print(
                (
                    "Epoch {} Training Cost {} Validation Precision "
                    "{} Regularization Cost {} Max Precision {}".format(
                        epoch,
                        train_cost / n_samples,
                        validation_cost,
                        regularization_cost,
                        maxp,
                    )
